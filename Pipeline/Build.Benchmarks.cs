...
partial class Build
{
	Target BenchmarkDotNet => _ => _
		.Executes(() =>
		{
			...
			DotNetBuild(s => s
				.SetProjectFile(Solution.Benchmarks.aweXpect_Chronology_Benchmarks)
				.SetConfiguration(Configuration.Release)
				.EnableNoLogo());

			DotNet(
				$"{Solution.Benchmarks.aweXpect_Chronology_Benchmarks.Name}.dll --exporters json --filter * --artifacts \"{benchmarkDirectory}\"",
				Solution.Benchmarks.aweXpect_Chronology_Benchmarks.Directory / "bin" / "Release");
		});
